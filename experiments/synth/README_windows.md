...
### R2b 第1ラウンド結果 (2026-07-06) — 判定基準未達

生成 16,128枚 (64シード×126構成, steps16, jscale2)、ゲート通過 14,855 (92.1%)。

| モデル | mydata OKS@50 (18枚) | chibi(3) | lineart(11) | part(4) | BP val AP |
|--------|------|------|------|------|------|
| Curriculum S2 (基準) | 0.548 | 0.501 | 0.568 | 0.530 | 0.834 |
| R2b BP-only (対照) | 0.550 | 0.501 | 0.576 | 0.514 | 0.833 |
| R2b mixed v1 (synth82%, 全cs) | **0.453** | 0.392 | 0.465 | 0.467 | 0.740 |
| R2b mixed v2 (synth50%, cs0.9のみ, gate≥0.95) | 0.550 | 0.435 | 0.572 | **0.575** | — |

- v1: 合成ドメインへの引っ張りで**全カテゴリ悪化**。混合比が支配的な失敗要因
- v2: 悪化は解消したが有意な改善なし (+0.002)。part (見切れ) のみ +0.045 の
  部分的シグナル (合成の得意分野: bust クロップ + 正確な v=0 教師と整合)。
  chibi は依然マイナス — 骨スケール chibi 素体の統計が実チビ絵と乖離している疑い
- **結論: 現状のスタイル現実度では転移しない。redesign_proposal §7 の分岐どおり
  「スタイルギャップが本丸」— 次はレンダリング/生成側の多様化に投資**:
  ①アナログ化後処理 (§3a-③: 紙テクスチャ・撮影シミュ — 未実装のまま),
  ②style LoRA 複数系統, ③スタイル距離の定量測定 (Danbooru sketch 参照セット),
  ④chibi はネイティブ頭身素体 (VRoid Studio) で作り直し。
  モデル側の調整はしない (これまでの教訓)
